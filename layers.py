"""
The lower lavel layers implementation of the models.
"""
...
class SelfAttention(nn.Module):
    """
        Self attention layer  with the function TimeDistributed provided by Allen NLP.
...
class BiDAFOutput(nn.Module):
    ...
    def __init__(self, hidden_size, drop_prob):
        ...
        self.rnn = GRUEncoder(input_size=2 * hidden_size,
    ...
    def forward(self, att, mod, mask):
        ...
        mask = mask.type(torch.float32)
        masked_logits_1 = mask * logits_1.squeeze(-1) + (1 - mask) * -1e30
        masked_logits_2 = mask * logits_2.squeeze(-1) + (1 - mask) * -1e30

        # Shapes: (batch_size, seq_len)
        # log_p1 = masked_softmax(logits_1.squeeze(), mask, log_softmax=True)
        # log_p2 = masked_softmax(logits_2.squeeze(), mask, log_softmax=True)

        return masked_logits_1, masked_logits_2
